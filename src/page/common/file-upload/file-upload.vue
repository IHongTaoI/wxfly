...
<div class="file-upload-container">
  ...
  <div class="crop-container" v-show="openCrop">
    <div class="crop-container__source" @touchend="mouseup" @touchmove="mousemove">
      <img class="crop-container__source__image" :src="imgSrc" ref="img"/>
      <div class="crop-box" :style="cropBoxStyle" @touchstart="mousedown" ref="cropbox">
    ...
    <canvas class="crop-canvas" ref="canvas" width="200" height="200" style="display: none;"></canvas>
...
export default {
  ...
  methods: {
    ...
    generateDrwaImageParam: function() {
      ...
      if (
        boxPosition.top >= imgPosition.bottom ||
        boxPosition.bottom <= imgPosition.top ||
        boxPosition.left >= imgPosition.right ||
        boxPosition.right <= imgPosition.left
      ) {
        return {
          source,
          dest
    ...
    mousedown: function(e) {
      ...
      this.enterPoint.y = e.touches[0].pageY;
    ...
    moveRightBottom: function(offsetX, offsetY) {
      ...
      if (
        this.cropBox.left + curW >= maxHeight ||
        this.cropBox.top + curH >= maxHeight
      ) {
        return;
      }
    ...
    moveLeftTop: function(offsetX, offsetY) {
      ...
      // 最小检查
      if (
        this.cropBox.height - offsetY <= minHeight ||
        this.cropBox.width - offsetX <= minHeight
      ) {
        return;
      }
      if (Math.abs(offsetY) > Math.abs(offsetX)) {
    ...
    move: function(offsetX, offsetY) {
      ...
      if (
        ofX >= 0 &&
        ofY >= 0 &&
        ofX + this.cropBox.width <= box.w &&
        ofY + this.cropBox.height <= box.h
      ) {
